...
export const onHabitCompletion = onDocumentCreated(
  ...
  async (event) => {
    ...
    const newXp = userSnap.data()?.xp || 0;
...
async function checkAndGrantBadges(db: FirebaseFirestore.Firestore, userId: string, habitId: string, streak: number, totalCompletions: number) {
  const badgesToCheck = [
